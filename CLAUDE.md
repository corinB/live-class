...
>   - Build: `cd live-class && ./gradlew build` (compile + bootJar — passes).
>   - Test: `./gradlew test` — currently fails with `ClassNotFoundException` on `LiveClassApplicationTests` due to Java 21 + Korean path interaction; non-blocking for design work. Documented in `plan/nested-launching-ripple.md`.
>   - Run (needs running Postgres): `./gradlew bootRun` → Swagger UI at `http://localhost:8080/swagger-ui.html`.
> - Pipeline state directories — `plan/before/` (pending tasks), `plan/after/` (worker-completed), `reports/` (per-agent end-of-run reports). Currently only `.gitkeep` files; populated as the pipeline runs.
> - User-facing replies are in **Korean**; technical artifacts (commits, planning docs, code comments) in **English**, per the parent `~/CLAUDE.md` global rule.
>
> The 10 behavioral rules below apply *on top* of the project-specific guidance above. When project conventions conflict with the rules, the project wins.

...
## Steps
1. [x] Inventory `docs/engineering/`: `git ls-files docs/engineering/` and `find docs/engineering/ -type f` (untracked are also moved).
2. [x] For each file, `git mv docs/engineering/<file> wiki-src/ko/docs/engineering/<file>` if tracked; `mkdir -p` + `mv` otherwise. Preserve filenames.
3. [x] Grep for the old paths repo-wide (excluding `.git/`): `grep -r "docs/engineering/" --include='*.md' --include='*.yml' --include='*.yaml' --include='*.json'`. Update every hit to `wiki-src/ko/docs/engineering/`.
4. [x] If any reference was inside `context.yaml`, update it (allowed because this is the "fix references" path, and the audit script will pass).
5. [x] Run `python .claude/scripts/audit-context-yaml.py` — must exit 0.
6. [x] Locally validate the YAML still parses.
7. [x] Save the migration manifest into `reports/38_Logic_Implementer_docs-engineering-backfill_2026-05-16.md`.
8. [x] Commit with footer `Refs: plan/before/38_Logic_Implementer_Docs_Engineering_Backfill.md`.
...
## Acceptance
- [x] `git ls-files docs/engineering/` returns nothing.
- [x] `wiki-src/ko/docs/engineering/` has the same file count as the source.
- [x] `python .claude/scripts/audit-context-yaml.py` exits 0.
- [x] No remaining `docs/engineering/` string anywhere except in `reports/` historical artifacts.

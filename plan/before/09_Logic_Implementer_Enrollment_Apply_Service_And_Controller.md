...
- [ ] `src/main/resources/lua/enrollment_apply.lua` — task 08 placeholder 를 본문으로 채움. 다음 로직.
  ```lua
...
- [ ] `src/main/resources/lua/enrollment_compensate.lua` — 본문 작성.
  ```lua
  ...
  return r1 + r2
  ```
- [ ] `application/enrollment/EnrollmentMirrorService.java` — `@Service`. Lua 실행 래퍼.
...
- [ ] `application/enrollment/EnrollmentApplicationService.java` — `@Service`.
  - `EnrollmentResponse apply(UUID classmateId, UUID classId, Instant now)` — `@Transactional(isolation=READ_COMMITTED, timeout=2)`.
    ...
    - `String luaResult = mirrorService.tryApply(classId, classmateId, appliedAtNanos, clazz.getCapacity().value());`
...
- [ ] (Verify) `application/enrollment/EnrollmentApplicationServiceTest.java` — `@IntegrationTest` (Testcontainers Postgres + Redis).
...
- [ ] (Verify) `web/enrollment/EnrollmentControllerSliceTest.java` — `@WebMvcTest`로 인증 헤더, body 검증, 응답 status 분기 확인 (service 는 `@MockBean`).
